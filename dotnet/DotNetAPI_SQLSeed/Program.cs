    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (users != null)
            {
                ...
                {
                    string sql = "SET IDENTITY_INSERT PortfolioProjectSchema.Users ON;"
                                    + "INSERT INTO PortfolioProjectSchema.Users (UserId"
                                    + ",FirstName"
                                    + ",LastName"
                                    + ",Email"
                                    + ",Gender"
                    ...
                    foreach (Users singleUser in users)
                    {
                        ...
                        if ((sql + sqlToAdd).Length > 4000)
                        {
                            ...
                            sql = "SET IDENTITY_INSERT PortfolioProjectSchema.Users ON;"
                                    + "INSERT INTO PortfolioProjectSchema.Users (UserId"
                                    + ",FirstName "
                                    + ",LastName"
                                    + ",Email"
            ...
            dataContextDapper.ExecuteSQL("SET IDENTITY_INSERT PortfolioProjectSchema.Users OFF");
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE PortfolioProjectSchema.UserSalary");

            if (userSalary != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO PortfolioProjectSchema.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                    foreach (UserSalary singleUserSalary in userSalary)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO PortfolioProjectSchema.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                        }
                        sql += sqlToAdd;
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE PortfolioProjectSchema.UserJobInfo");

            if (userJobInfo != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO PortfolioProjectSchema.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                    foreach (UserJobInfo singleUserJobInfo in userJobInfo)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO PortfolioProjectSchema.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                        }
