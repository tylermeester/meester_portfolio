...
public class ModifyAppointmentController implements Initializable {
    ...
    --------------------------------------------*/
    /**
     * Saves the user inputted appointment information and updates the appointment in the database. Start Time and
     ...
     * flexibility. Lambda function implemented to  iterate through allAppointments as a stream and add the
     * appointments which match the customerId, excluding the Appointment that is currently being modified.
     *
...
//            //Pre lambda function approach
//            //Loop through allAppointments to find all customerAppointments, excluding this appointment being updated
//            for (Appointment appointment : allAppointments) {
//                if ((appointment.getCustomerId() == customerId) && (appointment.getId() != appointmentId))
//                    customerAppointments.add(appointment);
//            }

            //Lambda function to iterate through allAppointments as a stream and add the appointments which
            // match the customerId, excluding the Appointment that is currently being modified
            allAppointments.stream().anyMatch(appointment -> {
                if((appointment.getCustomerId() == customerId) && (appointment.getId() != appointmentId)){
                    customerAppointments.add(appointment);}
                return false;
            });
            ...
            if (conflictingAppointments.isEmpty() && newStart.isBefore(newEnd)) {
                //Update appointment
                Appointment.updateAppointment(appointmentId, appointmentTitle, appointmentDescription, appointmentLocation, appointmentType,
    ...
    --------------------------------------------*/
    /**
     * Initializes the Modify Appointment page and populates the form with the selected Appointment's data and the
     * relevant ComboBox data. The User ComboBox is populated with all Users in the database and the Start/End Time
     * ComboBoxes are populated with the business's hours of operation (EST 8:00 - 22:00) in the user's local time.
     *
     * @param url URL
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        appointmentIdText.setText(String.valueOf(selectedAppointmentId));
        ...
        LocalDate selectedDate = selectedAppointment.getStartTimeLDT().toLocalDate();
        LocalTime selectedStartTime = selectedAppointment.getStartTimeLDT().toLocalTime();
        LocalTime selectedEndTime = selectedAppointment.getEndTimeLDT().toLocalTime();
        ...
        startTimeComboBox.getSelectionModel().select(selectedStartTime);
        ...
        endTimeComboBox.getSelectionModel().select(selectedEndTime);
        ...
        try {
            contactNameComboBox.getSelectionModel().select(Contact.getContact(contactId).getName());
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
